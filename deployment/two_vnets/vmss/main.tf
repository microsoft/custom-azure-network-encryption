...
resource "azurerm_public_ip" "vmss-pip" {
  name                         = "${var.vmss_prefix}-${var.resource_group_name}-pip"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_public_ip" "gateway-pip" {
  name                         = "${var.vmss_prefix}-${var.resource_group_name}-gateway-pip"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
  public_ip_address_allocation = "Dynamic"
  domain_name_label            = "${var.vmss_prefix}-gateway"
}
...
resource "azurerm_lb" "lb" {
  ...
  location            = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  depends_on          = ["azurerm_public_ip.vmss-pip"]
